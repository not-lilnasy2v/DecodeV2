...
public class Tel extends OpMode {
    ...
    private static final double[] RECOIL_OFFSETS = {0.0, 0.010, 0.022};
    private volatile double distantare, posU;
    private volatile long ultimaDistanta = 0;
            ...
            public void run() {
                while (!stop) {
                    ...
                    synchronized (blocat) {
                        ...
                        sIntake = Ipornit && !trageShooting && loculete < 3 && !gamepad1.b;
                        bApasat = gamepad1.b;
                    }

                    if (sIntake && !trageShooting) {
                        long now = System.currentTimeMillis();
                        if (now - ultimaDistanta >= 50) {
