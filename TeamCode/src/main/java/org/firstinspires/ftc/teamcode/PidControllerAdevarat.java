    ...
    private void calculate()
    {
        if (m_enabled)
        {
            ...
            if (Math.abs(m_error) > m_tolerance)
            {
                ...
                double potentialOutput = m_P * m_error + m_I * potentialTotalError + m_D * derivative;

                // Only accumulate if total output stays within bounds
                if (potentialOutput >= m_minimumOutput && potentialOutput <= m_maximumOutput)
                {
