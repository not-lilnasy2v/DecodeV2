...
public class TelR extends OpMode {
    ...
    private static volatile double TargetX = 144;
    ...
    private static final double[] RECOIL_OFFSETS_FAR   = {0.0, 0.025, 0.045};
    private static final double[] RECOIL_OFFSETS_CLOSE = {0.0, 0.015, 0.028};
    ...
    public void start() {
        ...
        Shooter = new Thread(new Runnable() {
            ...
            private void shootPattern() {
                ...
                double[] recoilOffsets = targetShooterVelocity >= 2000 ? RECOIL_OFFSETS_FAR : RECOIL_OFFSETS_CLOSE;
                int shotsFired = 0;
                scula.setPower(-1);
                bascula.setPosition(Pozitii.sede);
                ...
                for (int step = 0; step < 3; step++) {
                    ...
                    double recoil = recoilOffsets[Math.min(shotsFired, 2)];
            ...
            private void waitForShooterReady() {
                double tolerance = targetShooterVelocity * 0.07;
                double currentVoltage = Math.max(9.0, m.voltageSensor.getVoltage());
                long maxTimeout = targetShooterVelocity >= 2000 ? 400 : 200;
    ...
    private void trackLoop(Pose pose) {
        ...
        if (result != null && result.isValid()) {
            ...
            if (fiducials != null && !fiducials.isEmpty()) {
                ...
                double actualLatency = (result.getCaptureLatency() + result.getTargetingLatency()) / 1000.0;
                double angleAtCapture = currentAngle - Math.toDegrees(filteredHeadingRate) * actualLatency * SCALE_FACTOR;
