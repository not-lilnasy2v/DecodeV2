...
public class op extends OpMode {
    ...
    private static final double[] RECOIL_OFFSETS = {0.0, 0.020, 0.040};
    ...
    public static double FAR_FAST_FEEDER_POWER = -0.92;
    public static int FAR_FAST_FEED_MS = 42;
    public static int FAR_FAST_RESET_MS = 22;
    public static int FAR_FAST_SORT_EXTRA_MS = 8;
    public static double FAR_FAST_MIN_DROP = 85;   // cat lasi sa scada sub target
    public static int FAR_FAST_WAIT_MS = 140;      // wait maxim scurt pe bila 2/3
    public static int FAR_FAST_FIRST_WAIT_MS = 400;
    private enum ShotMode {
        NEAR,
        FAR
    }

    private volatile ShotMode shotMode = ShotMode.NEAR;
    ...
    public void start() {
        ...
        Shooter = new Thread(new Runnable() {
            ...
            private void waitForShooterReady() {
                double minAcceptable = targetShooterVelocity - 100;
                long timeout = System.currentTimeMillis() +1200;//800
                while (System.currentTimeMillis() < timeout) {
                    double v1 = Math.abs(m.shooter.getVelocity());
                    double v2 = Math.abs(m.shooter2.getVelocity());
                    if (v1 >= minAcceptable && v2 >= minAcceptable) return;
            ...
            /* private void waitForShooterReady() {
                 double tolerance = targetShooterVelocity - 100;
 ...
 */
            ...
            private double getMinShooterVelocity() {
                return Math.min(
                        Math.abs(m.shooter.getVelocity()),
                        Math.abs(m.shooter2.getVelocity())
                );
            }
            ...
            private void rapidFireShootFar() {
                int[] order = {0, 2, 1};
                int[] slots = new int[3];
                int n = 0;
                ...
                double originalPosU = posU;
                double lastSortPos = m.sortare.getPosition();
                double velScale = targetShooterVelocity / 1550.0;
                ...
                scula.setPower(FAR_FAST_FEEDER_POWER);
                bascula.setPosition(Pozitii.sede);

                for (int i = 0; i < n; i++) {
                    int slot = slots[i];

                    double recoil = RECOIL_OFFSETS[Math.min(i, 2)] * velScale;
                    m.unghiD.setPosition(originalPosU + recoil);

                    double sortTarget = getTarget(slot);
                    if (Math.abs(sortTarget - lastSortPos) > 0.001) {
                        m.sortare.setPosition(sortTarget);
                        m.kdf(m.sortareWaitMs(lastSortPos, sortTarget) + FAR_FAST_SORT_EXTRA_MS);
                        lastSortPos = sortTarget;
                    }

                    long start = System.currentTimeMillis();
                    long maxWait = (i == 0) ? FAR_FAST_FIRST_WAIT_MS : FAR_FAST_WAIT_MS;
                    double minAllowed = targetShooterVelocity - FAR_FAST_MIN_DROP;

                    while (getMinShooterVelocity() < minAllowed) {
                        if (System.currentTimeMillis() - start >= maxWait) break;
                        try {
                            Thread.sleep(2);
                        } catch (InterruptedException ignored) {}
                    }
                    ...
                    bascula.setPosition(Pozitii.lansareRapid);
                    m.kdf(FAR_FAST_FEED_MS);

                    bascula.setPosition(Pozitii.sede);
                    m.kdf(FAR_FAST_RESET_MS);
