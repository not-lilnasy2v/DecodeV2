...
@TeleOp(name = "stai parcat pzdo")
@Configurable
@Disabled
public class latraii extends OpMode {
    ...
    public volatile boolean turelaTracking = false, tracking = false, Ipornit = false,
            IntakePornit = false, SortingPornit = false, SortingToggle = false,
    ...
    private double EcuatieVelocity(double x) {
        double result = -0.00000516094 * Math.pow(x, 3)
                ...
                - 0.115998 * x
                + 3.332;
        return result * 1000;  // Conversia în ticks
    }
    ...
    private double CalculeVelocitate(double distanceInch) {
        double x = distanceInch;

        rawVelocity = EcuatieVelocity(x);
    ...
    private double CalculareCucUnghi(double distanceInch) {
        ...
        rawUnghidPos = EcuatieHoodAngle(x);
    ...
    private final Thread AutoShootUpdater = new Thread(new Runnable() {
        ...
        public void run() {
            while (!stop) {
                if (autoShootEnabled) {
                    // Calculeaza continuu distanța și ajustează
                    ShooterAdaptare();

                    // Seteaza unghiul hood continuu
                    m.unghiD.setPosition(autoHoodPosition);

                    // Seteaza velocity-ul shooter-ului continuu (doar când nu trage)
                    if (!trageShooting) {
    ...
    private final Thread Shooter = new Thread(new Runnable() {
                ...
                synchronized (blocat) {
                    ...
                    if (gamepad1.y && loculete > 0 && !sugere && !trageShooting) {
                        ...
                        if (autoShootEnabled) {
                            ...
                            else if (gamepad2.y) targetShooterVelocity = 2040;
                        }
        ...
        private void shootPattern() {
            ...
            for (int step = 0; step < 3; step++) {
                ...
                // Re-setează velocity înainte de fiecare bilă
                m.shooter.setVelocity(targetShooterVelocity);
                m.shooter2.setVelocity(targetShooterVelocity);
                asteaptaVelocity();
                shootBall();
...
//            m.Saruncare.setPosition(Pozitii.coborare);
    ...
    public void loop() {
        ...
        telemetry.addLine("");
        telemetry.addData(" Statusus", autoShootEnabled ? "pornit" : "oprit");
        telemetry.addData("Distance", String.format("%.1f inch", DistanceLaTarget));

        String rangeStatus;
        if (DistanceLaTarget < MinDistantaTragere) {
            rangeStatus = "prea aprope";
